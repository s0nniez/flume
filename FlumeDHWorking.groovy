 ...
 */	
		...
		valueTile("todayFlow", "device.todayFlow", decoration: "flat", height: 1, width: 2) {
        ...
        valueTile("currentFlow", "device.currentFlow", decoration: "flat", height: 1, width: 2) {
			state "currentFlow", label: 'Current: ${currentValue} gpm'
		}
        
		main "water"
		
        details(["water", /*"homeAway",*/ "suspend", "refresh", "todayFlow", "monthFlow", "yearFlow", "currentFlow"])
    }
    preferences {
...
def initialize() {
    ...
    schedule("0 0/1 * * * ?", poll) //refresh every 10 minutes changed to 1 
...
def refresh(){
    ...
    state.currentFlow = cloudData.currentFlow
    log.debug("device refresh  state.currentFlow '${state.currentFlow}'")
    ...
    sendEvent(name: "currentFlow", value: cloudData.currentFlow) //Math.round(cloudData.currentFlow * 100.000/100.000))
    /*
