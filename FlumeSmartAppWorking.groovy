...
log.debug("lastMinuteTime '${lastMinute}'")
        ...
        try {
               ...
               state.thisYearFlow = (resp_data.data.thisYear.value[0][0]).toInteger()
                ...
                //state.unitsFlow = resp_data?.units
               	...
               	state.currentFlow = (resp_data.data.currentMin.value[0][0])
                 log.debug("state.currentFlow '${state.currentFlow}'")
               // added current flow 12-4-19 s0nniez
            }
        } catch (e) {
...
Map retrievecloudData() {
   ...
   pollSLAlert()
	return ["todayFlow":state.todayFlow, "thisMonthFlow":state.thisMonthFlow, 
      "thisYearFlow":state.thisYearFlow, "currentFlow":state.currentFlow, "inAlert":state.inAlert]
}
